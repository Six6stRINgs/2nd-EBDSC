...
class ModernTCN(
    nn.Module
):  # T 在预测任务当中为预测的长度，可以更换为输出的种类 num_classes
    def __init__(
        self,
        M,
        num_classes,
        ...
        if stem:
            self.stem = nn.Sequential(
                nn.Conv1d(1, D, kernel_size=1, stride=1), nn.BatchNorm1d(D)
            )
        ...
        self.classificationhead = nn.Sequential(
            nn.Dropout(head_dropout), nn.Linear(D * M, num_classes)
        )
...
if __name__ == "__main__":
    ...
    # 对应的参数含义为 M, L, T, 4 个序列特征，96 原输入长度 96，预测输出长度为 192
    model = ModernTCN(5, 12, stem=True).cuda()
